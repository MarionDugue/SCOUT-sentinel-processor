...
def extract_wkt_from_kml(kml_path: str, layer: str = None) -> str:
    ...
    multipoly = gdf[gdf.geometry.type.isin(['Polygon', 'MultiPolygon'])].geometry.union_all()
...
def get_s1_data(wkt: str, config: dict) -> pd.DataFrame:
    ...
    naming_columns = ['satellite', 'mode', 'TTTR', 'polarisation', 'Start_DT', 'Stop_DT', 'Absolute_Orbit', 'Mission_ID', 'Product_ID']
    ...
    df.loc[df['satellite'] == 'S1A', 'relat_orbit'] = ((df['Absolute_Orbit'] - 73) % 175) + 1
    df.loc[df['satellite'] == 'S1B', 'relat_orbit'] = ((df['Absolute_Orbit'] - 27) % 175) + 1
