...
def download_scene(product_id: str, access_token: str, base_file_path: str, base_url: str) -> Optional[str]:
    ...
    with tempfile.NamedTemporaryFile(delete=False) as tmp_file:
        ...
        for chunk in tqdm(response.iter_content(chunk_size=block_size),
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Download Copernicus Sentinel-1 product by product id.")
    parser.add_argument("--product_id", required=True, help="Product ID to download")
    parser.add_argument("--config", required=True, help="Path to YAML config file")
    parser.add_argument("--output_dir", required=False, help="Override output directory to save downloads")
    args = parser.parse_args()

    config = load_config(args.config)
    creds = config.get("copernicus_credentials", {})
    s1_cfg = config.get("sentinel1", {})
    base_url = s1_cfg.get("download_url", "https://zipper.dataspace.copernicus.eu/odata/v1/Products({})/$value")
