...
def get_aoi_path(config):
    ...
    aoi_path = os.path.join(os.path.dirname(config_file), config['input']['aoi_path_total'])
...
def analyze_intersections(zip_path, aoi_path, polarization, log_file=None):
    ...
    try:
        ...
        try:
            ...
            # Validate AOI
            if aoi.empty:
                error_msg = f"Scene {scene_name}: AOI file is empty - no geometries found"
                logging.error(error_msg)
                return None
...
def main():
    parser = argparse.ArgumentParser(description='Find S1 burst/swath intersections with AOI')
